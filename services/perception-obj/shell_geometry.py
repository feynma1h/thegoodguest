"""Room-shell geometry assembly from ARKit plane anchors (decision 0066).

Turns the bundle's measured PlaneAnchor set into a floor quad plus wall
quads in the ARKit world frame — nothing is inferred: every emitted plane
is a merge of anchors ARKit actually detected, walls keep their DETECTED
extent (no extrapolation to floor or ceiling), and a bundle whose anchors
yield nothing usable returns an empty shell (the caller writes
status "unavailable").
...
Winding/orientation contract (consumed by shell_texture and the viewer):
each plane's corners are wound so cross(c1-c0, c3-c0) points along the
FRONT face — +Y (up) for the floor, the wall's detected normal for walls.
ARKit's vertical-plane normal (anchor +Y) points out of the wall toward
the camera that observed it, i.e. INTO the room, so front face = interior
side and single-sided rendering yields the dollhouse cutaway. corners[0]
is the texture origin; +U runs corner0→corner1, +V corner0→corner3.
...
Consumers: shell_receiver.py (the /shell stage), tests/test_shell_geometry.py.
"""
...
# sampling/budget knobs — expect a pass over these after the V3 walk)
# ---------------------------------------------------------------------------
...
class ShellPlaneGeom:
    """One assembled shell plane (floor or wall) in world coordinates."""

    kind: str  # "floor" | "wall"
    # (4, 3) world corners, wound so cross(c1-c0, c3-c0) is the front face.
    corners_world: np.ndarray
    normal: np.ndarray  # (3,) unit front-face normal
    origin: np.ndarray  # (3,) == corners_world[0]; texture (0, 0)
    axis_u: np.ndarray  # (3,) unit; texture +U, corner0 -> corner1
    axis_v: np.ndarray  # (3,) unit; texture +V, corner0 -> corner3
    width_m: float  # extent along axis_u
    height_m: float  # extent along axis_v
    classification: str | None  # majority-by-area member class, or None
...
class ShellGeometry:
    ...
    polygons (world frame) shell_texture rasterizes into the floor alpha."""
    ...
    # rectangles otherwise. Union defines the floor's alpha shape.
...
def _parse_anchor(index: int, anchor) -> _ParsedAnchor:
...
def _select_floor(parsed: list[_ParsedAnchor]) -> tuple[ShellPlaneGeom | None, list[np.ndarray]]:
    ...
    # these is the alpha shape and their XZ bbox is the quad.
    polygons = []
    for a in members:
    ...
    # Bake frame with cross(axis_u, axis_v) == +Y: u = +X, v = -Z, origin
    # at (min_x, y, max_z).
...
def _majority_classification(members: list[_ParsedAnchor]) -> str | None:
...
def _wall_frame(n: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def _mergeable(a: _ParsedAnchor, b: _ParsedAnchor) -> bool:
...
def _merge_wall_group(members: list[_ParsedAnchor]) -> ShellPlaneGeom:
    """One wall from coplanar-overlapping vertical anchors. Height is the
    DETECTED vertical extent of the members — never extrapolated."""
    total_area = sum(a.area_m2 for a in members)
    ...
    for a in members:
        ...
        n_sum += a.area_m2 * n
    normal, lateral = _wall_frame(n_sum / total_area)
...
def _merge_walls(parsed: list[_ParsedAnchor]) -> list[ShellPlaneGeom]:
    verticals = [
        a for a in parsed
    ...
    groups: dict[int, list[_ParsedAnchor]] = {}
...
def assemble_shell(plane_anchors) -> ShellGeometry:
    """Assemble the shell from a bundle's plane_anchors (repeated
    ...
    parsed = [_parse_anchor(i, a) for i, a in enumerate(plane_anchors)]

    floor, floor_polygons = _select_floor(parsed)
    walls = _merge_walls(parsed)
