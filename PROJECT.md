...
- **KNOWN GAP — staleness re-mint missing-file stall (unbuilt terminal state).** When the 12h staleness guard fires and a blob file is absent from Application Support, `onSessionExpired` calls `onFatalBlobError("blob_file_missing_at_staleness_remint")` and returns. But `onFatalBlobError` is an unbuilt stub (observability tracking + print only — no state mutation, no task cancellation, no record update). Consequence: the bundle stalls permanently with no terminal state, no UI/FCM surface, and no record cleanup. **SAFE:** bundle.pb is never finalized against the hole — the gate cannot read true with any blob whose pre-abort state remains in `sessionEntries`. **Pre-pass implemented:** the staleness branch now checks all non-bundle.pb file paths for existence before resetting any blob status or starting any upload task; a missing-file abort starts zero sibling uploads and mutates no store state. **Closure owner:** the real `onFatalBlobError` build (P5-adjacent terminal-state + FCM/UI surface), which must also cancel any in-flight sibling blob URLSession tasks from OTHER error paths (308/5xx retry) that do not share the pre-pass guard.
...
- **KNOWN BUG — `bundle.pb` strands `.pending` on relaunch after process death (Gate 2b NEGATIVE, root-caused; owned by P5).** On device (bundle `09c50526-…`): all frames uploaded and confirmed in GCS, `bundle.pb` PUT enqueued while alive, app swipe-up force-quit, locked ~5 min, reopened — `bundle.pb` never reached GCS, no `✗` fatal logged. Root cause (recon-confirmed, decision 0044): the recovery path (`beginUploadSession` → `onAllBlobsUploaded` → `enqueueBundlePb`) is correct but **unreachable on relaunch** — wired only to `ContentView.onChange(of: capture.bundlePath)` (fires on a new capture stop, not a bare reopen); `AppDelegate.handleEventsForBackgroundURLSession` only calls `setBackgroundCompletionHandler` and never reads the persisted record. Secondary OS-kill route: cancelled-task completion → `contexts[bundleId] == nil` → `onFatalBlobError` (unbuilt P5 stub, mutates nothing) → same stranded outcome but with a fatal *logged* (inferred, not yet observed). Structural: `BlobUploadStatus` has only `.pending`/`.uploaded` and cannot represent pending-but-orphaned. NOT a finalize regression (Gate 2 proves finalize works when its trigger fires) — a never-built launch-time recovery path. Owned by **P5** as a coupled cluster: (a) launch-time rehydration, (b) `onFatalBlobError` recovery to a retriable state, (c) idempotency hardening (one-shot Phase-1→2 latch + `enqueuePhasOneBlobs` in-flight guard + fix the unconditional `contexts[bundleId] = UploadContext()` overwrite that wipes first-wave retry/308 state) — where **(c) is a firm prerequisite for (a)**. Diagnostic branch `diag-bundlepb-reason-public` (`5bdd12f`) is parked as the tool to read the redacted fatal `reason=` when the OS-kill route is staged — do not delete.

## iOS test policy

The iOS suite is **115 tests total**, run manually via `xcodebuild … -scheme RoomStudioCapture-Integration` — the only scheme in this project (no separate default scheme, no CI gate). That scheme bakes `RUN_INTEGRATION_TESTS=1`, so the 4 `UploadSessionClientTests` **execute live on every run**; they are NOT skipped in practice. They ran live this P4 campaign against a reachable backend (`api-public-00006-quw`).
...
**Honest count:** report as "109 asserting offline unit tests + 2 boilerplate stubs (`testExample`/`testPerformanceExample`) + 4 live integration tests (require a reachable backend)", total 115 — not a bare total: the 2 stubs assert nothing and the 4 integration tests carry an external dependency the unit tests don't.
...
**1 — iOS P5 — scene polling + state UI, and the bundle.pb relaunch-recovery cluster** (decisions 0029, 0044). Two splits. (a) **Core poll + UI — buildable now:** `GET /scenes/by-bundle/{bundle_id}` on `api-public-00006-quw` (asia-southeast1); cadence 2s → 10s after 30s → cap 30s; poll only after `bundle.pb` is confirmed in GCS; 404 = keep polling. Terminal = `ready`/`failed_invalid`/`failed`; transient = `queued`/`processing`/`failed_incomplete`. Polling is authoritative; FCM is a later backend-gated accelerator (FCM `ready`/`failed` is currently broken — perception-obj uses `device_id` as if it were an FCM token; only `failed_incomplete` push works). Folds in the deferred read-path check: a `schema_version != "1"` bundle is rejected to `failed_invalid` (decision 0031) AND that Scene is readable via this GET (closes the read leg of the Gate-2 end-to-end smoke). (b) **The bundle.pb relaunch-recovery cluster** — headline gap from Gate 2b (see "What does NOT work" and decision 0044): launch-time rehydration + `onFatalBlobError` recovery + idempotency hardening, designed together, idempotency hardening a firm prerequisite for rehydration. P5 backend gaps (not P4): `invalid_blobs`/`InvalidBlobReason` exists server-side but isn't serialized into the GET; FCM `ready`/`failed` needs a real `fcm_token` plumbed at `/upload_session`.
