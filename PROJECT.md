...
- **Identity.** Firebase anonymous auth, upgraded in place by linking Apple
  ...
  a conflict is an explicit switch/keep choice. There is deliberately no iOS
...
Suite **544**: 538 asserting offline tests + 2 boilerplate stubs + 4 live
...
- `GET/POST /scenes/{id}/conversation` — the guest, at `FACTS_VERSION 3` and
...
- **The mask, not the crop, is what starves a reconstruction — measured
  causally on a GPU bench and NOT built (0198).** SAM 3D's input is RGBA with
  **alpha = the SAM mask** (`models/sam3d.py`), so an incomplete mask deletes
  from the model's input what the photograph actually contains. rp7 f114's mask
  excludes both desk legs; adding **~3,000 mask pixels** (IoU 0.949, same seed)
  takes the splat from `0.983 × 0.212 × 0.718` — a 21 cm slab that cannot be a
  desk under ANY rotation — to `0.983 × 0.655 × 0.444`, a body that renders as a
  desk with legs. Both halves are already available: incomplete masks are
  detectable from data the pipeline holds (unclaimed in-box LiDAR **40.3%/43.5%**
  on flagged frames vs **16.3%/19.1%** on controls, ~2.4×), and
  `Sam3Processor.add_geometric_prompt` is in the image we ship and never called.
  The contaminant is undetected OTHER objects, so the signal must be validated
  after use, never trusted blind. **The named bottleneck for a production
  version is the object-blind sampler** — one fixed object's good view was never
  sampled at all.
...
- **`scene_facts` still tells a rearranged room's guest that its facts were
  measured** (0174). Corrected downstream by the arrangement block, which is
  therefore load-bearing rather than belt-and-braces. Fixing it is a
  `FACTS_VERSION` bump.
...
- **`AccountConflictView` has never rendered.** The designed conflict screen with
  real room counts on both options is dead code; the shipped path is two stock
  alerts that state no counts. Either wire it or delete it.
...
- **There is no per-room deletion** — account deletion is all-or-nothing, which
  is conspicuous for a product whose thesis is that rooms are identity.
...
- **A registry-size check is owed.** After 2026-08-20 the repository should
  report ~132,000 MB, down from 1,477,656 MB. No movement at all means the
  background reclamation did not happen (0190).
...
- **App Store collateral is unstarted** except the icon: screenshots, support
  URL, age rating, and privacy nutrition labels (which must name the
  material-inference vision call).

### Blocked on something external

- **Apple Developer Program enrollment is stuck** — filed 2026-07-22, still
  unapproved against a typical <48 h. It gates Gate A (on-device Apple link),
  APNs, TestFlight, submission, and the 7-day personal-team re-sign treadmill
  that makes the device build expire. Worth contacting Apple Developer Support
  or checking for an identity-verification hold. Interim device builds need the
  two `CODE_SIGN_ENTITLEMENTS` lines dropped from `project.pbxproj` locally —
  **never commit that**.
- **Apple sign-in on the web** is enrollment-gated for the same reason. Google
  works; production-origin sign-in has been done with Google only.
...
- **`SceneStatusView` does not stand down when a new bundle starts uploading** —
  during a fresh capture's upload it can still show the prior scene's "Room
  ready". The new scene's poll runs correctly; the surface misleads.
...
than it looks like, and PROJECT.md's "cannot share an environment" is about the
...
root/api-public is 826+26 with it and 718+94 without; perception is 793+0 with
and 786+7 without. The fixture-backed tests skip silently. Always say which
...
`perception-geom` is outside every prefix and cannot be touched by it. The
DELETE carries no age condition on purpose — that is what makes the steady
state exactly 3 + 10 + 10 + 1 images (≈129 GiB) rather than "that plus
whatever is recent".
...
**Standards the repo is held to.** No intermediate-dev references, no placeholder
...
the code and PROJECT.md, the story of how it changed lives in `docs/decisions/`.
...
action. Everything else it should read from PROJECT.md and `docs/decisions/`,
...
home by the session that found it: a decision note, or PROJECT.md. Never left
...
**Decision numbers.** **Always derive the free list from `ls docs/decisions/`, not from this paragraph** — it has lagged three times. As of 2026-08-20, with the clipped-views lane merged and **nothing reserved**: **free are 0113, 0121, 0128, 0134, 0144, 0145, 0167, 0168, 0186, 0189, 0192, 0193, 0194, 0195, 0196, 0204+**; **0199–0200 are reserved to the colour-deploy lane and 0201–0203 to object-aware-sampling**. Spent by clipped-views: **0197** (the uncropped photograph is not a better photograph) and **0198** (the mask is the photograph SAM 3D sees); **0199** was reserved to that lane and released unused, as were **0195–0196** by disk-cleanup. Everything else through 0198 is used.
