...
- **iOS P5 relaunch-recovery cluster code-complete** (decisions 0044/0045; (c) `5bb07c9`, (b) `cc7aba5`, (a) `658cc4a`, test-pin `bd8b86f`). Three coupled units: (c) idempotency hardening — `bundlePbEnqueueInFlight` in-process latch + `getAllTasks` cross-process reconciliation + context-preserve fix in `enqueuePhasOneBlobs`; (b) `onFatalBlobError` reclassified into TERMINAL/DEFERRED-INTERRUPTED/DEFERRED-TRANSIENT with `.failed` + `failureReason` persisted on terminal paths, sibling-task cancellation, and bounded cross-launch retry (N=10 per-launch, reset on progress); (a) launch-time rehydration via `rehydrateAllUnfinishedBundles()` triggered from `.task` in `RoomStudioCaptureApp`, with pre-checks for missing blob files before any enqueue. Suite 162-green incl. 4 live integration tests. **Hardware gate pending:** the OS-kill route (background URLSession relaunch, `.task` fires without entering foreground) is unverified on-device — code-complete ≠ Gate-2b closed. See "What does NOT work" and decision 0045.
...
The iOS suite is **162 tests total**, run manually via `xcodebuild … -scheme RoomStudioCapture-Integration` — the only scheme in this project (no separate default scheme, no CI gate). That scheme bakes `RUN_INTEGRATION_TESTS=1`, so the 4 `UploadSessionClientTests` **execute live on every run**; they are NOT skipped in practice. They ran live this P5(a) campaign against a reachable backend (`api-public-00006-quw`).
...
**Honest count:** report as "156 asserting offline unit tests + 2 boilerplate stubs (`testExample`/`testPerformanceExample`) + 4 live integration tests (require a reachable backend)", total 162 — not a bare total: the 2 stubs assert nothing and the 4 integration tests carry an external dependency the unit tests don't.
...
**1 — iOS P5 — OS-kill hardware gate + scene polling + state UI** (decisions 0029, 0044, 0045). Two pieces: (a) **OS-kill hardware gate (cluster close item):** stage on-device: all blobs uploaded, `bundle.pb` PUT enqueued while alive, force-quit, reopen — confirm `bundle.pb` reaches GCS with no user interaction (GCS-authoritative, verified from Mac during locked interval); also verify `.task` fires on background OS-relaunch. `diag-bundlepb-reason-public` (`5bdd12f`) is the parked tool for reading the redacted `reason=`. AppDelegate fallback (decision 0045 Fork A) deferred pending this gate. (b) **Core poll + UI — buildable now:** `GET /scenes/by-bundle/{bundle_id}` on `api-public-00006-quw` (asia-southeast1); cadence 2s → 10s after 30s → cap 30s; poll only after `bundle.pb` is confirmed in GCS; 404 = keep polling. Terminal = `ready`/`failed_invalid`/`failed`; transient = `queued`/`processing`/`failed_incomplete`. Polling is authoritative; FCM is a later backend-gated accelerator (FCM `ready`/`failed` is currently broken — perception-obj uses `device_id` as if it were an FCM token; only `failed_incomplete` push works). Folds in the deferred read-path check: a `schema_version != "1"` bundle is rejected to `failed_invalid` (decision 0031) AND that Scene is readable via this GET. P5 backend gaps (not P4): `invalid_blobs`/`InvalidBlobReason` exists server-side but isn't serialized into the GET; FCM `ready`/`failed` needs a real `fcm_token` plumbed at `/upload_session`.
