...
- **`PoseExtractor.gravity(from:)` is a zero-vector stub.** Implementation and test must be routed for review before P2 commits this field — it is a coordinate-frame derivation where a wrong answer (wrong rotation direction, wrong sign of world-down) produces a valid unit vector that passes a norm check. The test must pin hand-computed camera-local gravity vectors for two known orientations (camera upright/horizontal → (0,−1,0); camera pitched 90° looking straight down → (0,0,−1)) and not use a self-referential round-trip. See locked requirements from 2026-05-30 session.
...
**1 — iOS P2: tier dispatch + full proto assembly + `bundle.pb`.** P1 complete. P2 assembles a real `CaptureBundle` proto in memory: detect LiDAR capability → set `CaptureTier`, populate all fields including `schema_version = "1"`, implement `PoseExtractor.gravity(from:)` after review, serialize to `bundle.pb` on disk. Two gates before P2 merges: (a) `schema_version = "1"` enforcement in `api-internal` ingest handler (board item 2) must ship + be tested + be deployed first; (b) resolve the two P1 carry-overs (gravity formula + Sendable warnings) at P2 pre-flight.
