...
depend on how many controls a screen has. `tools/ios_screenshot_gallery.py`
photographs all 33 surfaces and `tools/ios_layout_audit.py` measures them; it
exits non-zero when an enforced screen deviates. Last run: **0 of 28**, margin
25–29pt, header ink 81–89, first content 140–148, filled button 77–82pt off the
bottom at left 27–28 and 346–348 wide.
...
- **The pinned-action rule reached ONE screen out of eleven when measured**
  ...
  is why the suite is green and why reading did not catch them. **Deliberately
  NOT fixed here**: four layout changes across four files, landing mid-brand-lane,
  on screens the home-sectioning work may recompose anyway. The two that show a
  user no action at all — the rooms list and the failure screen — depend on no
  open design decision and can be fixed on their own.
