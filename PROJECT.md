...
- **happy-path and duplicate-event smoke modes are not verified to `ready`.** The synthetic fixture (`tools/upload_test_bundle.py`) carries non-decodable placeholder pixels in its image blobs; SAM 3D loads successfully (P1 resolved 2026-05-29) but reconstruction fails at "Frame 0 image cannot be opened." This is by design — reaching `ready` requires real image data, deferred to the iOS client. The planned ingest validation gate (see board) will fast-fail these bundles at the ingest layer, making `failed_invalid` the correct terminal state for smoke happy-path (not `ready`). Until that gate ships, happy-path polls until timeout.
...
**1 — Build the ingest validation gate** (services/api-internal `/ingest/eventarc` handler). A bundle with present-but-non-decodable image blobs currently travels the full GPU path before failing at reconstruction. The gate validates image decodability (cheap, pre-GPU) and fails fast to a new `failed_invalid` Scene state. Once shipped, happy-path smoke reaches `failed_invalid` deterministically (~3 s), making Phase 7 4/4 green. Then Phase 8c (iOS code start) follows. See "What does NOT work" for the design contract.
