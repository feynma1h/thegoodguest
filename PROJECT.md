...
This file is the always-current state of the project. If something in here is wrong, fix it before doing anything else.
...
- **Smoke tool pass 3 scoping locked** (see `docs/decisions/0017`): manifest derivation, upload sequencing, and PUT semantics are pinned against the current `/upload_session` contract. Recon from a prior session covers the handler, request/response schemas, GCS minting, and Firestore session repo.
- **Smoke tool pass 4 scoping locked** (see `docs/decisions/0019`): polling contract pinned against a new `GET /scenes/by-bundle/{bundle_id}` endpoint on api-public. Recon from prior session covers the Scene document lifecycle, field population at ingest, idempotency under at-least-once Eventarc delivery, and the existing `SceneRepository` patterns in api-internal.
...
- **Two-service iOS upload path deployed to `asia-southeast1`:** `api-public` (revision `api-public-00006-quw`, 100% traffic, Firebase verifier wired) and `api-internal` (revision `api-internal-00013-qeg`, 100% traffic, Cloud Run IAM gated; carries the decision 0031 schema_version fix). `/health` 200 on both.
...
- **The review-findings-cleanup branch is NOT merged or deployed.** Production still runs the pre-cleanup revisions (`api-internal-00013-qeg`, `api-public-00006-quw`, `perception-obj-00024-89b`). The branch carries breaking contract changes: `device_id` is required (bundles without it → `failed_invalid`), `POST /ingest` no longer exists, and perception-obj's notifier signatures renamed `device_id` → `fcm_token`. Merge + redeploy all three services before the next smoke run; `tools/build_test_bundle.py` already sets `device_id`.
...
The iOS suite is **187 tests total** (was 191; the cleanup pass removed 5 legacy-decode tests whose migration shims were deleted and added 1 strict-decode pin), run manually via `xcodebuild … -scheme RoomStudioCapture-Integration` — the only scheme in this project (no separate default scheme, no CI gate). That scheme bakes `RUN_INTEGRATION_TESTS=1`, so the 4 `UploadSessionClientTests` **execute live on every run**; they are NOT skipped in practice. They last ran live 2026-07-17 (cleanup pass) against a reachable backend (`api-public-00006-quw`).
...
**Honest count:** report as "181 asserting offline unit tests + 2 boilerplate stubs (`testExample`/`testPerformanceExample`) + 4 live integration tests (require a reachable backend)", total 187 — not a bare total: the 2 stubs assert nothing and the 4 integration tests carry an external dependency the unit tests don't.
...
**Session-end housekeeping commits:** the PROJECT.md updates and any decision-note additions from session-end housekeeping should land in their own commit, separate from the code changes that prompted them. Message convention: `docs: session housekeeping — <one-line summary of what changed>`.

**What's gitignored:** see `.gitignore`. Notably: `outputs/`, virtualenvs and `.env`. Everything under `docs/decisions/` IS tracked: it is project documentation, not local state.
...
**1 — Merge + deploy the review-findings-cleanup branch** (17 commits, breaking backend contract changes — see What-does-NOT-work). Redeploy `api-internal`, `api-public`, and `perception-obj`; re-run the smoke modes (`tools/upload_test_bundle.py`, all four; note `--use-hardware-id-fallback` now exercises the `device_id_missing` rejection path). This precedes any new smoke run or iOS upload against production.

**2 — iOS P5 — OS-kill hardware gate** (decisions 0029, 0044, 0045). Core poll + status UI shipped (P5(a), commit `4556e44`). Remaining: **OS-kill hardware gate (cluster close item):** stage on-device: all blobs uploaded, `bundle.pb` PUT enqueued while alive, force-quit, reopen — confirm `bundle.pb` reaches GCS with no user interaction (GCS-authoritative, verified from Mac during locked interval); also verify `.task` fires on background OS-relaunch. `diag-bundlepb-reason-public` (`5bdd12f`) is the parked tool for reading the redacted `reason=`. AppDelegate fallback (decision 0045 Fork A) deferred pending this gate. After hardware gate: FCM `ready`/`failed` — backend threading is done (cleanup pass: Scene.fcm_token → ClaimResult → notifiers), so what remains is iOS-side FCM registration and passing the real `fcm_token` at `/upload_session`; plus relaunch/cold-start poll recovery (ScenePoller.onAppear finds `.complete` records; relaunch seam is there but the poller is not restarted across launches yet).

**3 — LiDAR-tier hardware verification (full-P4-close blocker for LiDAR tiers)** — needs a Pro device. ARKIT_ONLY green does NOT certify `captureDepth()`, LiDAR-tier dispatch, or the decision-0032 depth-intrinsics derivation (zero execution on a non-LiDAR device). The on-device runbook is parametric with `[LIDAR-UNVERIFIED]` tags so the same runbook re-runs on a Pro device. Subsumes the prior LIDAR_ARKIT depth-residual check (decision 0033).
...
**1. PROJECT.md updates.** Show a proposed diff for:
...
- Strategy already captured in PROJECT.md — skip.
...
PROJECT.md is the present. Anything historical — abandoned approaches, the reasoning behind a current convention, "we tried X and it didn't work" — goes in `docs/decisions/`, not here. If you find yourself wanting to add "we used to do Y" to PROJECT.md, that's a signal to write a decision note instead and keep PROJECT.md focused on what's true now.
