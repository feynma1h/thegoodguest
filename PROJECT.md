...
**1 — Phase 8c: begin iOS code.** The ingest validation gate is shipped (board item 1 done), Phase 7 is 4/4 green. happy-path and duplicate-event both terminate at `failed_invalid` deterministically, confirming pre-GPU fast-fail. iOS development can begin; real bundles from the iOS app are expected to reach `ready`.
