...
# ---------------------------------------------------------------------------
# Room shell fixture (shell.json v2, decision 0069) — kept in sync with
# mock.ts's shell doc. Parametric materials only: nothing is written to
# disk for the shell and nothing joins asset_urls.
# ---------------------------------------------------------------------------

def _material(family, albedo_hex, roughness, observed, **params) -> dict:
    return {
        "family": family,
        "family_confidence": 0.82 if family else None,
        "albedo_hex": albedo_hex,
        "secondary_hex": None,
        "params": params,
        "render": {"roughness": roughness},
        "source": {
            "observed_fraction": observed,
            "texel_count": int(observed * 12000),
            "frames_used": 6 if observed > 0 else 0,
        },
        "inference": {
            "model": "claude-sonnet-5" if family else None,
            "material_version": 1,
        },
    }
...
def _edges(**overrides) -> dict:
    edges = {
        name: {"state": "observed", "extension_m": 0}
...
def write_shell_fixture() -> dict:
    """Return the v2 shell doc. Geometry mirrors mock.ts: an L-ish floor
    polygon at y=0 (exercises triangulation), a back wall at z=-2.6 with a
    door opening + closure provenance, a side wall at x=-2.2 fully
    UNOBSERVED (albedo null — the neutral-treatment state). Corners wound
    with the front face toward the room interior."""
    floor_polygon = [[-2.2, 0, 1.4], [0.6, 0, 1.4], [0.6, 0, 0.9],
                     [1.8, 0, 0.9], [1.8, 0, -2.6], [-2.2, 0, -2.6]]
    shell = {
        "shell_version": 2,
        "scene_id": SCENE_ID,
        "status": "ready",
        ...
        "floor": {
            ...
            "y": 0,
            "provenance": {"edges": ["observed"] * len(floor_polygon)},
            "material": _material("wood", "#8a6f52", 0.6, 0.62,
                                  plank_direction_deg=90),
        },
        "walls": [
            {
                "wall_id": "wall_00",
                ...
                "measured_quad": [[-2.2, 0.3, -2.6], [1.8, 0.3, -2.6],
                                  [1.8, 2.2, -2.6], [-2.2, 2.2, -2.6]],
                "edges": _edges(
                ...
                "openings": [
                    {"classification": "door",
                     "rect_uv": [[0.62, 0], [0.84, 0.86]]},
                ],
                "classification": "wall",
                "material": _material("painted", "#c9b9a4", 0.85, 0.55),
            },
            {
                "wall_id": "wall_01",
                "quad": [[-2.2, 0, 1.4], [-2.2, 0, -2.6],
                         [-2.2, 2.2, -2.6], [-2.2, 2.2, 1.4]],
                "measured_quad": [[-2.2, 0, 1.4], [-2.2, 0, -2.6],
                                  [-2.2, 2.2, -2.6], [-2.2, 2.2, 1.4]],
                "edges": _edges(),
                "openings": [],
                "classification": None,
                "material": _material(None, None, 0.9, 0.04),
            },
        ],
        "quality": {"planes_in_bundle": 4, "frames_used": 9,
                    "material_version": 1, "fragments_dropped": 1},
    }
    return {"shell": shell, "urls": {}}
...
def main() -> None:
    ...
    shell_fixture = write_shell_fixture()
