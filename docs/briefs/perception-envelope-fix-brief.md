...
findings behind each item are recorded in PROJECT.md's envelope bullet and the
first-real-capture bullet. Consumer: the session that fixes perception's
...
Delete this file when the fix ships — PROJECT.md and the fix session's decision
...
Convention:  See PROJECT.md. Tests pin invariants, not implementation.
