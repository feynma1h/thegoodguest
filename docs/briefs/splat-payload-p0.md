...
**Status:** SERVING as of 2026-08-09/10 — the four-surface deploy (`api-public-00034-zad`, hosting 23:31:04, `api-internal-00023-mek`, `perception-obj-00038-7b5`). Deploy findings in decisions 0106–0109. Remaining: the operator's two-minute production browser check (PROJECT.md's P0 bullet).
...
exists and gates the object wave per piece. Both remaining items from this
paragraph's earlier form are DONE: `asset_urls_compressed` serves, and
`/compress` runs as a third `/process` stage so new captures get the tier
at birth.
...
- the JSX-entity quirk in `web/README.md` is invisible to eslint, tsc, and the
