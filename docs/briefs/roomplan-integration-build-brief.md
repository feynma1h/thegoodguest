...
numbers become regression-pin tolerances in RP-4/RP-2. Consumer: the session(s) that build it. Chunks RP-0..RP-5 are server/schemas work
verifiable OFFLINE against the spike fixture before any iOS work; RP-6/7
are iOS; RP-8 is the deploy + operator gate. Hand over with the usual adjustment: the docs half is ALREADY DONE (decision
0077 + the PROJECT.md delta land with the design session), so implementing
...
```
Read PROJECT.md first. Decision 0077 is the design
...
Task:        Build the LIDAR_ROOMPLAN tier end-to-end. Server first (all
             ...
             server chunks (separate session/worktree per PROJECT.md).
...
Convention:  See PROJECT.md + decision 0077. Tests pin invariants at
             ...
             gotcha in memory), rebase + hand-merge PROJECT.md bullets,
