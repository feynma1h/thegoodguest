...
shell (decision 0066: walls + floor from measured ARKit planes, baked
real-pixel textures, a second perception stage).

Produced by the 2026-07-22 room-shell design session (branch
room-shell-design), which locked the forks in decision 0066. Consumer: the
session(s) that build the shell — the chunks below are separable
(chunk A is capture-side and can ride an iOS session; B and C are backend/
web). Hand over with one adjustment: the docs
half is ALREADY DONE (decision 0066 and the PROJECT.md delta land with the
design branch), so implementing sessions skip straight to the build.

Delete this file when the shell ships — the durable record is decision 0066.
-->

# Build brief — room shell (decision 0066)

```
Read PROJECT.md first. Decision 0066 is the design
...
Verify-first (do these BEFORE building the dependent parts; each kills the
             ...
             UNPROVEN in our tree. If it fails, stop and re-open 0066's
             ...
             main empirical unknown (0066 records the re-open condition).
...
Constraints: Chunk A: packages/schemas/capture_bundle.proto (additive field
             ...
             enrollment provisioning (see PROJECT.md's 0051 bullet).
...
Contract:    Proto (additive): message PlaneAnchor {Pose pose (world_from_
             ...
             prerequisite (folds in PROJECT.md's audit item): identify the
             ...
             "capture_expired", method: "arkit_planes" (ladder per 0066:
             ...
             pacing tunable at the real-browser watch PROJECT.md already
...
Verify by:   Python (per-directory, per PROJECT.md's testpaths note): shell
...
Convention:  See PROJECT.md and decision 0066. Tests pin invariants, not
             ...
             main — rebase, and hand-merge the PROJECT.md bullets). No merge,
