...
(decision 0085 + PROJECT.md), per the build-brief deletion convention.
...
Roles: **[OP]** = operator with phone. **[MAC]** = the session on this Mac
...
2. [MAC] `pull.sh store` → diff against the pre-install listing. Expect:
...
1. [MAC] `stage.sh '{"corruptFrame": "frames/000000.jpg"}'`
2. [OP] Scan a room SMALL (one slow 15–20 s sweep), Finish → review → **Send
   it home**. Stay on the wait screen.
3. [MAC] Watch the bundle in GCS; on bundle.pb landing, `scene_status.py` —
...
1. [MAC] `stage.sh '{"fatalBlob": "frames/000003.jpg"}'`
...
3. [MAC] Verify sibling cancellation: GCS prefix stops growing; breadcrumbs
   show the fake-404.
...
5. [MAC] `pull.sh store` — record gone (uploadFailed reclaim row). Because
...
1. [MAC] `stage.sh '{"suspendBundlePb": true}'`
2. [OP] Scan small, Send it home, LEAVE THE PHONE (wait screen showing).
3. [MAC] Watch GCS until all frames/ blobs are present and breadcrumbs show
...
5. [MAC] Confirm bundle.pb still absent (~30 s watch).
6. [OP] Unlock, **reopen the app** — touch NOTHING else.
7. [MAC] Watch GCS: bundle.pb must land within ~30 s with zero user
...
1. [MAC] `stage.sh '{"exitAfterCompletions": 25}'`
...
3. [MAC] Watch GCS: remaining blobs keep landing AFTER the process death
...
5. **Free Live Activity check — the feature's headline claim, and this is the
   ...
   count; [MAC]: compare it against the GCS prefix at the same moment. A frozen
...
- [OP] AirDrop screenshots #1–#3 to this Mac (→ ~/Downloads; [MAC] moves them
  to `outputs/release-residue-sitting/`).
- [MAC] Final pulls (store + breadcrumbs), scene statuses for runs A/B/D/E,
  write decision 0085 (Gate 2b + Fork A verdicts), update PROJECT.md, delete
  this brief.
