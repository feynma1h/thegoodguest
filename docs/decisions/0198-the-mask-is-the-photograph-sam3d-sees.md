...
Same photograph, same seed, ~3,000 mask pixels of legs different: the slab
becomes an open frame with legs and cross-rails standing on the box floor —
which is what the real desk is. 0.144 is the best residual any arm of this
desk has ever measured (shipped f7: 0.633). Seed variance is real but small
(0.236 vs 0.356, both slabs) and cannot explain the jump.
...
- `splat_axis_resolved` went False on the refined arm; if axis resolution
