...
1. Both the implementation of `gravity(from:)` and its test must be routed
   for review before P2 commits the gravity field. This is a
   coordinate-frame derivation; per PROJECT.md, wrong answers propagate.
...
2. The test must assert against camera-local ground-truth values derived
   ...
   the floor). The specific expected vectors are derived and reviewed
...
derived and reviewed (not pre-stated), and why self-referential
