...
- **Putting the partial Info.plist inside the RoomStudioCapture/ synchronized
  ...
  build setting — the RoomStudioCapture.entitlements precedent — avoids the
...
- `RoomStudioCapture-Info.plist` at the project top level, containing ONLY
...
- `test_appBundle_registersRedirectSchemeForLiveClientID` is THE drift pin:
  ...
  CLIENT_ID is absent, per the worktree note in PROJECT.md's iOS test policy.
...
- **The OAuth client is regenerated** (new client ID): the drift pin goes
  red naming the fix — update RoomStudioCapture-Info.plist's scheme and
...
- **A second URL scheme joins the app** (deep links, the QR bridge's real
  ...
  onOpenURL handler in RoomStudioCaptureApp then needs routing beyond the
