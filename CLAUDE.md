...
**The deferred-import smoke hardening is BUILT and must NOT ship alone.** 0211's defect was `mask_refine.py` imported unconditionally by `process_receiver` and never COPY'd into the image, which made `main` undeployable while every test passed. The ship lane's guard pinned that ONE module; a follow-up generalises it to every module any function imports, and lives on the auto-named branch `claude/upbeat-faraday-73b2a6` — one commit, `Dockerfile` and `test_dockerfile_manifest.py`. **It busts the layer cache from its step down, so it belongs on the next perception build rather than a build of its own.** Nothing on `main` references that branch, so it is one cleanup away from being lost.
