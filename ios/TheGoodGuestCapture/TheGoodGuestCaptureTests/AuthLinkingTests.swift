...
///     (TheGoodGuestCapture-Info.plist) and the GITIGNORED client ID
...
final class AuthLinkingTests: XCTestCase {
    ...
    /// (TheGoodGuestCapture-Info.plist) while the client ID it derives from is
    ...
    func test_appBundle_registersRedirectSchemeForLiveClientID() throws {
        guard
            ...
            throw XCTSkip("no GoogleService-Info.plist in the app bundle (worktree note in PROJECT.md)")
        }
        ...
        XCTAssertTrue(
            ...
            TheGoodGuestCapture-Info.plist to match the plist's \
